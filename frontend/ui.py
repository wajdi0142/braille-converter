...
logging.basicConfig(filename='qt_errors.log', level=logging.WARNING, 
...
class BrailleUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        table_layout.addWidget(self.line_width_label)
    ...
    def keyPressEvent(self, event):
        ...
        if key in (Qt.Key_Return, Qt.Key_Enter):
            ...
            # Insérer un saut de ligne à la position actuelle
            cursor.insertText("\n")
        ...
        # Gestion des touches fléchées pour éviter des conversions inutiles
    ...
    def adjust_line_width(self):
        ...
        if ok:
            ...
            if tab:
                ...
                # Reformater chaque ligne individuellement
    ...
    def _restore_cursor_position(self, text_edit, cursor_pos):
        ...
        position = min(cursor_pos, len(text))
        cursor.setPosition(position)

        # Ajuster la position après un saut de ligne
        lines = text.split('\n')
        current_pos = 0
        target_line, target_col = self._get_cursor_position_info(text, cursor_pos)
        for i, line in enumerate(lines):
            if i + 1 < target_line:
                current_pos += len(line) + 1  # +1 pour le '\n'
            else:
                current_pos += min(target_col, len(line))
                break
    ...
    def sync_text_areas(self, tab):
        ...
        try:
            ...
            # Vérifier si le changement est juste un espace ou un saut de ligne
            ...
            if is_minor_change:
                ...
                if text_changed and not braille_changed:
                ...
                elif braille_changed and not text_changed:
    ...
    def _convert_to_braille(self, tab, current_input):
        ...
        if len(current_input) <= 500:
            # Conserver les sauts de ligne et reformater chaque ligne
    ...
    def test_conversion(self):
        ...
        tests = {
            "Français (grade 1)": "Voici la version mise à jour, intégrant l'analyse. C'est un test.",
    ...
    def _save_or_export(self, tab, file_path=None, export_format="txt", prompt_save_type=True):
        ...
        # Déterminer le nom du fichier pour le titre du document
    ...
    def print_braille(self):
        ...
        try:
            ...
            # Utiliser le nom du fichier si disponible, sinon le titre de l'onglet
            tab_title = self.tab_widget.tabText(self.tab_widget.indexOf(tab))
